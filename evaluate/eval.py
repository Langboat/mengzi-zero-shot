import sys
import time
from load_data import eprstmt_dataset, tnews_dataset, lcqmc_dataset, cluner_dataset, finre_dataset, cote_dataset, cepsum_dataset, kuake_qic_dataset, express_ner_dataset
...
datasets = {'sentiment_classifier': eprstmt_dataset(),
            ...
            'company_extraction': cluner_dataset()}
...
for task_name in task_name_list:
    ...
    if task_name in ['sentiment_classifier', 'news_classifier', 'text_similarity', "medical_domain_intent_classifier"]:
    ...
    elif task_name in ["entity_extraction", "company_extraction"]:
        cvt_res = []
        ...
        if task_name == 'company_extraction':
            _, f1_score_avg = ner_get_f1_score(pre_lines=cvt_res, gold_lines=list(df['label']), labels=['公司'])
        else:
